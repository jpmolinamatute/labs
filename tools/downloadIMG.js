...
let i = 0;
list.forEach((l) => {
    // db.pokemonlist.update({name: l.name}, {$set: {detailpageurl: l.detailpageurl, thumbnailimage: l.thumbnailimage}});
    i += 1;
    if (typeof l.detailpageurl === 'string' && !alreadyIN.includes(l.name)) {
        alreadyIN.push(l.name)
        txt += `"https://www.pokemon.com${l.detailpageurl}" `;
    }
});
txt += ')';
console.log('%s', txt);

console.log(`Number of Items in list: ${i}`);
console.log(`Number of added to alreadyIN: ${alreadyIN.length}`);
